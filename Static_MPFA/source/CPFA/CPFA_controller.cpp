...
#include <unistd.h>
#include<iostream>
CPFA_controller::CPFA_controller() :// constructor
	...
	SIR_state(SUSCEPTIBLE),//HUI 0926
	LoopFunctions(NULL),
...
void CPFA_controller::Init(argos::TConfigurationNode &node) {//read from xml? parameters about actuars and sensors
 ...
 if(controllerID=="CPFA_11"){
 	m_pcLEDs->SetAllColors(CColor::RED);
 	std::cout<<"SET INFECTEDD"<<std::endl;
 	SIR_state=INFECTED;
 }else{
 	m_pcLEDs->SetAllColors(CColor::GREEN);
 }
 //std::cout<<controllerID<<std::endl;
 //m_pcLEDs->SetAllColors(CColor::RED);//hui
 //std::cout<<"this is init method"<<std::endl;
}
...
void CPFA_controller::Reset() {
 isHoldingFood   = false;
    ...
    ResourceDensity = 0;
    collisionDelay = 0;
    std::cout<<"this is reset method"<<std::endl;
...
bool CPFA_controller::SetTargetPheromone() {
	...
	if(ClosestNest->isInfected==true && randomNumber>0.1){
